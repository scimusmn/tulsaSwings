obtain(['µ/utilities.js'], ({ averager, map, distance, clamp })=> {
  ...
  exports.func = (swing, audio)=> {
    ...
    if (swing.totalWeight() > 15 && !swing.active) {
      swing.active = true;
      //audio.forEach((track)=> {track.rampUp();});
      swing.lastHigh = Date.now();
    } else if (swing.lastHigh && Date.now() - swing.lastHigh > 1000) {
      swing.active = false;
      audio.forEach((track)=> {track.rampDown();});
      //audio[0].rampDown();
      //swing.setLights(map(deg, 0, 360, 0, 127), 0, audio[0].volume * 127);
    } else if (swing.totalWeight() > 15) {
      audio.forEach((track, i)=> {
        var seg = (360 / audio.length);
        var zone = seg * (i + .5);
        var level = clamp(.5 - dist / 2, 0, 1);
        var spec = clamp((seg - Math.abs(zone - deg)) / seg, 0, 1);
        spec += clamp((seg - Math.abs(zone - (deg - 360))) / seg, 0, 1);
        spec += clamp((seg - Math.abs((zone - 360) - deg)) / seg, 0, 1);
        level = clamp(level + spec, 0, 1);

        track.volume = level;
        //console.log(i + ' ' + track.volume);
      });
  ...
  var playAll = ()=> {
    var loaded = exports.tracks.reduce((acc, cur)=>acc && cur.loaded, true);
    if (loaded) exports.tracks.forEach((track, i)=> {
      track.currentTime = 0;
      track.play();
    });
  };
